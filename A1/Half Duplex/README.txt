Half Duplex Socket Communication allows the client(on a local system) and server(on a cloud) to communicate but only one at a time. 

It is a two way connection/communication where at a given time only one user can transmit while others receive. 
